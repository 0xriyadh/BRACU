...
{
       0,    56,    56,    63,    69,    77,    83,    91,    91,   110,
     110,   128,   139,   150,   161,   174,   174,   200,   200,   218,
     235,   243,   251,   261,   278,   296,   313,   334,   341,   350,
     357,   365,   372,   379,   386,   393,   400,   407,   414,   423,
     430,   439,   447,   456,   463,   472,   479,   488,   495,   504,
     512,   521,   529,   539,   546,   553,   562,   569,   576,   583,
     590,   597,   604,   613,   621,   629,   636
};
#endif
...
#line 91 "22241157.y"
			...
			if(!is_function_declared(yyvsp[-3]->getname())) {
				vector<pair<string, string> > params = current_func_params;
				symbol_info* func = new symbol_info(yyvsp[-3]->getname(), "ID", yyvsp[-4]->getname());
				func->set_as_function(yyvsp[-4]->getname(), params);
				table->insert(func);
			}
...
#line 110 "22241157.y"
                                                  {
			// Create and insert function symbol before compound statement
			if(!is_function_declared(yyvsp[-2]->getname())) {
...
#line 262 "22241157.y"
            ...
            if(is_variable_declared_current_scope(yyvsp[0]->getname())) {
                // Silently ignore multiple declarations
                yyval = new symbol_info(yyvsp[-2]->getname() + "," + yyvsp[0]->getname(), "decl_list");
            } else {
                // Create and insert new variable
                symbol_info* new_var = new symbol_info(yyvsp[0]->getname(), "ID", current_type);
...
#line 279 "22241157.y"
            ...
            if(is_variable_declared_current_scope(yyvsp[-3]->getname())) {
                // Silently ignore multiple declarations
                yyval = new symbol_info(yyvsp[-5]->getname() + "," + yyvsp[-3]->getname() + "[" + yyvsp[-1]->getname() + "]", "decl_list");
            } else {
                // Create and insert new array
                int size = stoi(yyvsp[-1]->getname());
                symbol_info* new_array = new symbol_info(yyvsp[-3]->getname(), "ID", current_type, size);
